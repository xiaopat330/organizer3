...
public class JdbiRatingCurveRepository implements RatingCurveRepository {

    private final Jdbi jdbi;

    @Override
    public Optional<RatingCurve> find() {
        return jdbi.withHandle(h ->
                h.createQuery("SELECT global_mean, global_count, min_credible_votes, cutoffs_json, computed_at FROM rating_curve WHERE id = 1")
                        .map((rs, ctx) -> RatingCurve.fromRow(
    ...
    @Override
    public void save(RatingCurve curve) {
        jdbi.useHandle(h ->
                h.createUpdate("""
                        INSERT INTO rating_curve (id, global_mean, global_count, min_credible_votes, cutoffs_json, computed_at)
