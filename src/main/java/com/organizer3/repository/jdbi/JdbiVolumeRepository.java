...
public class JdbiVolumeRepository implements VolumeRepository {
    ...
    private static final RowMapper<Volume> MAPPER = (rs, ctx) -> {
        Volume v = new Volume(
                rs.getString("id"),
                Path.of(rs.getString("mount_path")),
    ...
    public void save(Volume volume) {
        jdbi.useHandle(h ->
                h.createUpdate("""
                        INSERT INTO volumes (id, mount_path, structure_type, last_synced_at)
                        VALUES (:id, :mountPath, :structureType, :lastSyncedAt)
                        ...
                        .bind("mountPath", volume.getMountPath().toString())
