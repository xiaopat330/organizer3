// utilities-javdb-discovery/enrich-panels.js — Shared Titles + Profile panel
// mounts for the v1 (legacy) surface.
//
// Parallel to v2/discovery/enrich-panels.js.  Logic is identical; import paths
// differ because v1 lives in a different module tree.
//
// Public API:
//   mountTitlesPanel(containerEl, { actressId, hooks?, showActionBar? })
//   mountProfilePanel(containerEl, { actressId, hooks? })
//
// showActionBar (default true): when false, the Enrich/Stop buttons are omitted.
// Discover passes false because its action bar lives outside the subview div.
...
export function mountProfilePanel(containerEl, { actressId, hooks = {} }) {
  ...
  async function render() {
    try {
      ...
      if (res.status === 404) { await render404(); return; }
