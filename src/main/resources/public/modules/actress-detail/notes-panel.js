// notes-panel.js — Sticky-note panel for the v1 actress detail left column.
//
// Usage (called once after renderDetailPanel):
//   mountActressNotePanel(actressId);
//
// Renders into #actress-detail-note-panel, which is appended as the last
// child of .actress-detail-left if it does not already exist.
//
// Spec: §5.3 of PROPOSAL_POST_IT_NOTES.md
//   - Yellow sticky surface (--postit-yellow), ink text (--postit-ink)
//   - Soft drop shadow (--postit-shadow)
//   - Slight rotation (--postit-rotation, ≈ -1deg)
...
const PANEL_ID  = 'actress-detail-note-panel';
const MAX_CHARS = 280;

// ── Relative-time helper (epoch millis → "edited X ago") ─────────────────
//
// Produces the "edited …" phrasing expected by §5.3.
// Does not import utils.js timeAgo() because that expects ISO strings;
// the note's updated_at is epoch millis.
//
...
// Absolute timestamp for the tooltip (matches fmtDate/locale convention in utils.js)
...
//
// getOrCreatePanel() finds or inserts the stable #actress-detail-note-panel
// div as the last child of .actress-detail-left. The container persists
// across actress navigations; mountActressNotePanel() re-renders its contents.
//
function getOrCreatePanel() {
  ...
  if (!panel) {
    ...
    panel.id = PANEL_ID;
    const col = document.querySelector('.actress-detail-left');
...
//
// Called from actress-detail.js after renderDetailPanel(data).
...
export async function mountActressNotePanel(actressId) {
  ...
  try {
    ...
    console.error('[notes-panel] getNote failed:', err);
...
function renderPresent(panel, actressId, noteState) {
  ...
  // Escape body for safe HTML insertion.
...
function renderEdit(panel, actressId, existingNote) {
  ...
  // Cancel: revert to previous view.
  cancelBtn.addEventListener('click', () => {
    detachCounter();
    panel.className = prevClass;
    panel.innerHTML = prevHtml;
    // Re-attach the click listener on the reverted view.
    if (existingNote) {
  ...
  saveBtn.addEventListener('click', async () => {
    ...
    try {
      ...
      console.error('[notes-panel] putNote failed:', err);
  ...
  if (clearBtn) {
    clearBtn.addEventListener('click', async () => {
      ...
      try {
        ...
        console.error('[notes-panel] deleteNote failed:', err);
