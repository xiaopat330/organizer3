// v2 Custom Avatar Editor modal.
// Full port of legacy modules/custom-avatar-editor.js.
//
// Reuses the .cae-overlay / .cae-modal DOM structure (from details.css) rather
// than the .modal-backdrop primitive — the canvas cropper requires fixed sizing
// that the v2 modal primitive (max-width:520px, 80vh) is too small for.
// All .cae-* styles are already loaded on v2-actress-detail.html via details.css.
//
// Entry point:
//   openCustomAvatarEditor(actressId, hasCustomAvatar, onDone)
//
// API contracts mirrored from legacy verbatim:
//   GET    /api/actresses/{id}/title-covers
//   GET    /api/actresses/{id}           (re-derive hasCustomAvatar on Back)
//   POST   /api/actresses/{id}/custom-avatar   body: image/jpeg blob
//   DELETE /api/actresses/{id}/custom-avatar
...
function initCropper(body, actressId, img, coverUrl, close, onDone) {
  ...
  // Defer size computation until after the canvas is in the DOM and
  // the body has layout.
      ...
      try {
        ...
        if (!res.ok) {
          ...
          if (res.status === 400) {
            ...
            msg = (body2 && (body2.error || body2.message))
              ? (body2.error || body2.message)
