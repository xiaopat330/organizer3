...
export function renderTitleCard(t, opts = {}) {
  const { variant = 'standard', watched = false, aging = false } = opts;
  const compact = variant === 'compact';

  const code    = t.code || '';
  const cover   = coverUrl(t);
  const name    = displayName(t);
  const actress = !compact ? (t.actressName || (t.actresses && t.actresses.length ? t.actresses[0].name : '')) : '';
  ...
  if (compact) {
    el.innerHTML = `
      ...
      ${date ? `<div class="tcv2-meta"><span class="tcv2-year">${esc(String(date).slice(0, 4))}</span></div>` : ''}
    `;
  } else {
    el.innerHTML = `
      <div class="tcv2-cover${cover ? '' : ' tcv2-cover--empty'}"
