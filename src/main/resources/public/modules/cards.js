...
export function makeActressCard(a) {
  ...
  if (!a.localAvatarUrl && covers.length > 0) {
    const badge = document.createElement('button');
    badge.type = 'button';
    badge.className = 'actress-avatar-badge';
    badge.title = 'Add profile image';
    badge.innerHTML = CARD_USER_CIRCLE_SVG;
    badge.addEventListener('click', e => {
      e.preventDefault();
      e.stopPropagation();
      openCustomAvatarEditor(a.id, !!a.hasCustomAvatar, async () => {
        try {
          const res = await fetch(`/api/actresses/${a.id}`);
          if (!res.ok) return;
          const updated = await res.json();
          const existing = document.querySelector(`.actress-card[data-actress-id="${a.id}"]`);
          if (existing) existing.replaceWith(makeActressCard(updated));
        } catch (_) { /* ignore */ }
      });
    });
